...
vec3 getColor(vec3 normal, vec3 pos) {
    float x = abs( pos.x - floor( pos.x ) - 0.5 ) * 2.;
    float y = abs( pos.y - floor( pos.y ) - 0.5 ) * 2.;
    
    return ( mix( v_vColour.rgb, iColor2, x ) + mix( v_vColour.rgb, iColor3, y ) ) / 2.;
}
...
vec2 coord =-1.0+2.0*v_vTexcoord.xy/iResolution.xy;
coord.x *= iResolution.x/iResolution.y;

// Get direction for this pixel
vec3 rayDir = normalize(camDir + (coord.x*camRight + coord.y*camUp)*FieldOfView);

gl_FragColor = vec4( rayMarch(camPos, rayDir, v_vTexcoord ).rgb, v_vColour.a );
}
